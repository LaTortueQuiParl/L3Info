...
void Cylindre::ColorCylindre(float red, float green, float blue){
    for (int i = 0; i < this->nombrePoints; i++){
        glBegin(GL_POLYGON);
        glColor3f(red,green,blue);  glVertex3f(this->pCylindre[i].getX(), this->pCylindre[i].getY(), this->pCylindre[i].getZ());
        glColor3f(red,green,blue);  glVertex3f(this->pCylindre[(i+1)%this->nombrePoints].getX(), this->pCylindre[(i+1)%this->nombrePoints].getY(), this->pCylindre[(i+1)%this->nombrePoints].getZ());
        glColor3f(red,green,blue);  glVertex3f(this->pCylindre[(i+1)%this->nombrePoints+this->nombrePoints].getX(), this->pCylindre[(i+1)%this->nombrePoints+this->nombrePoints].getY(), this->pCylindre[(i+1)%this->nombrePoints+this->nombrePoints].getZ());
        glColor3f(red,green,blue);  glVertex3f(this->pCylindre[i+this->nombrePoints].getX(), this->pCylindre[i+this->nombrePoints].getY(), this->pCylindre[i+this->nombrePoints].getZ());
        glEnd();
    }
...
void Cylindre::appliTexture(){

    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, LTexture, HTexture, 0, GL_RGB, GL_UNSIGNED_BYTE, texture);
...
void Cylindre::setTexture(char* fichier, int l, int h){
